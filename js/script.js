...
function initPagination(category, perPage = 6) {
  const grid = document.getElementById(`${category}-grid`);
  if (!grid) return;
  
  const items = grid.querySelectorAll('.before-after-slider');
...
function updateGallery(category) {
  ...
  // Update text label (e.g., "1 / 3")
  const totalPages = Math.ceil(state.totalItems / state.perPage) || 1;
  document.getElementById(`${category}-page-info`).innerText = `${state.current} / ${totalPages}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Sets to 6 items per page (3 columns, 2 rows)
  initPagination('detailing', 6);
  initPagination('gardening', 6);
});
